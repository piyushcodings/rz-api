...
async def _resolve_site_to_razorpay(client: httpx.AsyncClient, site_url: str, common_headers: dict):
    try:
...
# Main Check Function
async def razorpay_check_api(cc: str, mm: str, yy: str, cvv: str, site: str, proxy: Optional[str] = None, amount: str = "100"):
    start = time.time()
    ...
    async with httpx.AsyncClient(
        ...
        **transport_kwargs,
        ...
        if not data:
            ...
            return {
                "status": "error",
                "message": "Razorpay data not found",
                "elapsed_seconds": elapsed
            }
        ...
        if not key_id or not key_id.startswith("rzp_"):
            ...
            if key_match:
                ...
                return {
                    "status": "error",
                    "message": "No live Razorpay key found",
                    "elapsed_seconds": elapsed
                }
        ...
        if not payment_page_id:
            ...
            return {
                "status": "error",
                "message": "Payment page item ID not found",
                "elapsed_seconds": elapsed
            }
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Order creation failed: {str(e)[:50]}",
                "elapsed_seconds": elapsed
            }
        ...
        if isinstance(order_data, dict):
            ...
            if not order_amount or order_amount == amount_str:
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"IIN lookup failed: {str(e)[:50]}",
                "elapsed_seconds": elapsed
            }
        ...
        if not iin_data or "error" in iin_data:
            ...
            if "invalid" in err_desc.lower() or not err_desc:
                return {
                    "status": "declined",
                    "message": "Invalid Card / BIN rejected",
                    "elapsed_seconds": elapsed
                }
            return {
                "status": "declined",
        ...
        if not country or not network:
            ...
            return {
                "status": "declined",
                "message": "Card network/country not found",
                "elapsed_seconds": elapsed
            }
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Forex check failed: {str(e)[:50]}",
                "elapsed_seconds": elapsed
            }
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Payment submit failed: {str(e)[:50]}",
                "info": info_str,
                "elapsed_seconds": elapsed
            }
            ...
            if isinstance(err, dict):
                ...
                if err_source == "bank" or err_step == "payment_authorization":
                    if err_reason in live_reasons or any(r in err_desc.lower() for r in ["insufficient", "do not honor", "lost", "stolen", "cvc", "cvv"]):
                        return {
                            "status": "approved",
                            "message": f"{err_desc[:60]} (Live Declined)",
                            "info": info_str,
                            "elapsed_seconds": elapsed,
                            "details": {"reason": err_reason, "source": err_source}
                        }
                    ...
                    return {
                        "status": "approved",
                        "message": f"Bank Declined: {err_desc[:50]}",
                        "info": info_str,
                        "elapsed_seconds": elapsed,
                        "details": {"reason": err_reason, "source": err_source}
                    }
                ...
                if err_code == "BAD_REQUEST_ERROR" and "trouble" in err_desc.lower():
                    return {
                        "status": "error",
                        "message": "Site rejected payment (try different razorpay.me link)",
                        "info": info_str,
                        "elapsed_seconds": elapsed
                    }
        ...
        if razorpay_payment_id:
            return {
                "status": "charged",
                "message": f"Payment OK ({razorpay_payment_id[:20]})",
                "info": info_str,
                "elapsed_seconds": elapsed,
...
# API Endpoints
@app.get("/check", response_model=CardCheckResponse)
async def check_card_get(
    cc: str = Query(..., description="Card data in format: number|mm|yy|cvv"),
    site: str = Query(..., description="Razorpay site URL or razorpay.me link"),
    proxy: Optional[str] = Query(None, description="Proxy URL (e.g., http://user:pass@host:port)"),
    amount: str = Query("100", description="Payment amount")
):
    """
    Check a credit card against Razorpay payment gateway using GET method.
    
    Card format: cc|mm|yy|cvv (e.g., 4111111111111111|12|25|123)
    
    Returns status: approved, declined, charged, or error
    """
    # Parse card data from pipe-separated format
    card_parts = cc.split("|")
    if len(card_parts) != 4:
        raise HTTPException(status_code=400, detail="Invalid card format. Expected: cc|mm|yy|cvv")
    
    card_num, mm, yy, cvv = card_parts
    
    # Validate card number is numeric
    if not card_num.isdigit():
        raise HTTPException(status_code=400, detail="Card number must contain only digits")
    
    result = await razorpay_check_api(
        cc=card_num,
        mm=mm,
        yy=yy,
        cvv=cvv,
